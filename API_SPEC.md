# API Spec

## 필수 기능 대비 현재 상태

- 회원가입/로그인: `users`, `auth_sessions` 테이블로 구현 가능하며 API를 추가했습니다.
- 게시판: 기존 DB에 테이블이 없어 `board_posts`, `board_post_comments`, `board_post_files`를 추가했습니다.
- 파일 업로드: `files` 테이블을 사용하며 로컬 저장소와 DB 메타데이터 저장 API를 추가했습니다.
- AI 결과 연동: `model_runs`, `permit_analysis_results`, `risk_scores`에 모델 실행/분석/위험도 결과를 저장하는 API를 추가했습니다.
- 대시보드/시연: `work_permits`, `safety_events`, `risk_scores` 기반 요약 API를 추가했습니다.

## 추가로 있으면 좋은 것

- 실제 인증/인가 고도화: 현재는 DB 세션 토큰 기반입니다. 운영 수준에서는 JWT access token, refresh rotation, 권한별 접근 제어가 필요합니다.
- 게시판 운영 기능: 게시글 수정/삭제, 공지 고정, 검색, 페이지네이션, 관리자 숨김 처리가 추가되면 좋습니다.
- 파일 저장소: 현재 로컬 디렉터리에 저장합니다. 클라우드 배포 시 S3/Object Storage로 교체하는 구성이 필요합니다.
- AI 비동기 처리: 모델 호출이 오래 걸릴 수 있으므로 큐, 상태 업데이트, 재시도, 실패 로그가 있으면 안정적입니다.
- 테스트 데이터/시드: 발표 시연을 위해 사이트, 사용자, 작업허가, 위험 이벤트 샘플 데이터가 필요합니다.

## 주요 엔드포인트

```text
GET  /api/health

POST /api/auth/register
POST /api/auth/login
POST /api/auth/logout

POST /api/files
GET  /api/files/{id}
GET  /api/files/{id}/download

GET  /api/board/posts?category=general
GET  /api/board/posts/{id}
POST /api/board/posts
POST /api/board/posts/{id}/comments

GET  /api/master/sites
POST /api/master/sites
GET  /api/master/blocks
GET  /api/master/cameras

GET  /api/work-permits
GET  /api/work-permits/{id}
POST /api/work-permits

GET  /api/safety-events
POST /api/safety-events

GET  /api/ai/model-runs
POST /api/ai/model-runs
POST /api/ai/work-permits/{permitId}/analysis-results

GET  /api/risks/scores
POST /api/risks/scores
POST /api/risks/simulations

GET  /api/dashboard/summary
```

인증이 필요한 API는 로그인 응답의 `accessToken`을 아래처럼 전달합니다.

```http
Authorization: Bearer {accessToken}
```

...
function Page({page,role,session,notify}) {
  ...
  return page==="monitoring"?<Monitoring notify={notify}/>:page==="permits"?<Permits notify={notify}/>:page==="risk"?<Risk/>:page==="audit"?<Audit/>:<DigitalTwin session={session} notify={notify}/>;
}
